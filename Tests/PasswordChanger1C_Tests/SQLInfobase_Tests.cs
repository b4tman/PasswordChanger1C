...
namespace PasswordChanger1C.Tests
{
    ...
    {
        const string PostgresPassword = "1";

        private byte[] FromBase64(in string data)
        {
            var replaces = "\n,\r, ,\t".Split(',');
        ...
        public void GetAll_PostgreSQL_Test()        
        {
            ...
            readerMock.Setup(reader => reader.GetValue(0)).Returns(FromBase64("k42MsT0XVTlFDlfbSk/naQ==")); //ID
            readerMock.Setup(reader => reader.GetString(1)).Returns("938d8cb13d175539450e57db4a4fe769");  //IDStr
            ...
            readerMock.Setup(reader => reader.GetValue(4)).Returns(FromBase64(@"
            ...
            Assert.Collection(result, x => {
                ...
                var ExpectedHashes = CommonModule.GeneratePasswordHashes(PostgresPassword);
